...
void send_ping_request(evutil_socket_t sock, struct in_addr addr, long long * time_before_send) {
  ...
  *time_before_send = get_time_in_microseconds();
...
void recv_icmp_reply(evutil_socket_t sock, short event, void * arg) {
  ...
  if (icmp->icmp_type != ICMP_ECHOREPLY) {
    fprintf(stderr, "strange reply type (%d)\n", icmp->icmp_type);
    return;
  }
  ...
  fprintf(stderr, " Correct ICMP echo reply; payload size %zd content %.*s\n", data_len,
...
void calculate_icmp_delay_for_addr(struct in_addr addr) {
  ...
  for (i = 0; i < nr_of_peers; ++i) {
    if (peers[i].ip_addr.s_addr == addr.s_addr) {
      found = 1;
      long long act_time = get_time_in_microseconds();
      long long diff = act_time - peers[i].icmp_delay.time_in_usec_before_send;
      int index = peers[i].icmp_delay.nr_of_measurements % 10;
      long long add_to_sum = 
        diff - peers[i].icmp_delay.last_measurements[index];
      peers[i].icmp_delay.last_measurements[index] = diff;
      ...
      fprintf(stderr, "Delay calculated for icmp for ip: %s, nr of measurements: %d,"
              " avg_delay in usec based on last 10 measurements: %f\n", 
              inet_ntoa(addr), peers[i].icmp_delay.nr_of_measurements, peers[i].icmp_delay.avg_delay);
    }
  }
  if (!found) {
